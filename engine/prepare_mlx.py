...
MLX port — uses numpy/mlx instead of torch for data storage.

Usage:
    uv run prepare_mlx.py                  # full prep (download + tokenizer)
    uv run prepare_mlx.py --num-shards 2   # download only 2 shards (for testing)
...
EVAL_TOKENS = 3 * 524288 # reduced for faster iteration on Apple Silicon
...
# ---------------------------------------------------------------------------
# Runtime utilities (imported by train_mlx.py)
# ---------------------------------------------------------------------------
...
def get_token_bytes():
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing token_bytes lookup at {path}. Run prepare_mlx.py first.")
...
def _document_batches(split, tokenizer_batch_size=128):
    ...
    assert len(parquet_paths) > 0, "No parquet files found. Run prepare_mlx.py first."
