...
Based on trevin-creator/autoresearch-mlx.
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        x = self.c_fc(x)
        x = mx.maximum(x, 0) ** 2  # ReGLU squared activation
...
class GPT(nn.Module):
    ...
    def __call__(self, idx, targets=None, reduction="mean"):
        ...
        logits = 15.0 * mx.tanh(logits / 15.0)  # softcap
...
# ---------------------------------------------------------------------------
# JSON output mode for subprocess communication
# ---------------------------------------------------------------------------

def emit_json(event_type, **kwargs):
    """Emit a structured JSON event to stdout for the Node.js parent process."""
...
if __name__ == "__main__":
    ...
    if json_mode:
        emit_json("log", text="🚀 Loading tokenizer and data...", logType="info")
    ...
    t_data = time.time()
    print(f"Data/tokenizer loaded in {t_data - t_start:.1f}s", file=sys.stderr)
    ...
    if json_mode:
        emit_json("log", text=f"🧠 Model: {num_params/1e6:.1f}M params, DEPTH={DEPTH}, dim={model_dim}", logType="info")
        emit_json("log", text=f"⏱️ Time budget: {TIME_BUDGET}s, Grad accum: {grad_accum_steps}", logType="info")
    else:
        print(f"Time budget: {TIME_BUDGET}s", file=sys.stderr)
        print(f"Gradient accumulation steps: {grad_accum_steps}", file=sys.stderr)
    ...
    while True:
        ...
        for _ in range(grad_accum_steps):
            ...
            if t_compiled is None:
                ...
                print(f"Model compiled in {t_compiled - t_data:.1f}s", file=sys.stderr)
            train_loss = loss
        ...
        if train_loss_f > 100:
            if json_mode:
                emit_json("log", text="❌ Training loss exploded — FAIL", logType="error")
        ...
        # Emit progress every 10 steps in JSON mode
        if json_mode and step % 10 == 0:
            emit_json("progress",
                step=step,
                progress=round(pct_done, 1),
    ...
    if json_mode:
        emit_json("log", text="📊 Starting final evaluation...", logType="info")
    ...
    if json_mode:
        emit_json("result",
            ...
            trainingSeconds=round(total_training_time, 1),
            totalSeconds=round(t_eval - t_start, 1),
            peakMemoryMb=round(peak_vram_mb, 1),
            totalTokensM=round(total_tokens / 1e6, 1),
            numSteps=step,
            numParamsM=round(num_params / 1e6, 1),
            depth=DEPTH,
        )
    else:
        print("---", file=sys.stderr)
        print(f"val_bpb:          {val_bpb:.6f}", file=sys.stderr)
        print(f"training_seconds: {total_training_time:.1f}", file=sys.stderr)
        print(f"total_seconds:    {t_eval - t_start:.1f}", file=sys.stderr)
        print(f"peak_vram_mb:     {peak_vram_mb:.1f}", file=sys.stderr)
        print(f"total_tokens_M:   {total_tokens / 1e6:.1f}", file=sys.stderr)
        print(f"num_steps:        {step}", file=sys.stderr)
        print(f"num_params_M:     {num_params / 1e6:.1f}", file=sys.stderr)
